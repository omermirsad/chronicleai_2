...
interface JournalEntryCardProps {
  ...
  // Fix: Add optional onDelete prop
...
const JournalEntryCard: FC<JournalEntryCardProps> = ({ entry, onOpenPerspectiveLens, onDelete }) => {
  ...
  useEffect(() => {
    // Trigger animation for the card itself on mount
    const mountTimer = setTimeout(() => setIsMounted(true), 10);
    return () => clearTimeout(mountTimer);
  }, []);

  useEffect(() => {
    // Trigger fade-in animation for the AI analysis footer when it becomes available
    if (entry.aiAnalysis) {
