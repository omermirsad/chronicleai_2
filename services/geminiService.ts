...
export const analyzeEntry = async (text: string, photo?: { base64: string; mimeType: string }): Promise<AIAnalysis> => {
  try {
    ...
    // Fix: Use correct model name 'gemini-2.5-flash' and API call structure
    ...
    // Fix: Use response.text to get content, as per guidelines
...
const generatePerspective = async (title: string, prompt: string): Promise<Perspective> => {
    try {
        ...
        // Fix: Use response.text to get content
    ...
    if (history.length === 0) {
        prompt = `You are a gentle and insightful journaling guide. Give me a single, welcoming, open-ended starting prompt for a journaling session about "${sessionType}". Provide ONLY the text of the prompt.`;
         try {
            ...
            // Fix: Use response.text and trim whitespace
            return [response.text.trim()];
        } catch (error) {
            console.error("Error generating initial guided prompt:", error);
        ...
        try {
            ...
            console.error("Error generating guided prompts:", error);
             // Provide fallback questions that still offer choice to maintain the UX
            return [
