import { FC, useState, useEffect, useCallback, ChangeEvent, FormEvent, cloneElement } from 'react';
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-stone-200">
        ...
        {history.map((turn, index) => (
          <div key={index} className="mb-4">
            <div className="bg-amber-50 text-amber-900 p-4 rounded-md prose" dangerouslySetInnerHTML={{ __html: marked.parse(turn.prompt) as string }}></div>
        ...
        {currentPrompt && !isThinking && (
          <div className="bg-amber-50 text-amber-900 p-4 rounded-md mb-4 prose" dangerouslySetInnerHTML={{ __html: marked.parse(currentPrompt) as string }}></div>
        )}
                  ...
                  {[1, 2, 3, 4, 5].map((level) => {
                    ...
                    return (
                      <button
                        ...
                        className={`p-2 rounded-full text-3xl transition-all duration-200 ${mood === level ? 'scale-125' : 'scale-100 opacity-50 hover:opacity-100 hover:scale-110'
                          }`}
