...
export const useAuth = () => {
  ...
  useEffect(() => {
    ...
    const getSession = async () => {
      try {
        // Add timeout to session fetch (30 seconds)
        const sessionPromise = supabase.auth.getSession();
        const result = await withTimeout(sessionPromise, 30000);
        ...
        // If operation timed out, clear session and let user log in again manually
        // avoiding the "Connection Error" blocker for auth stalls
        if (mounted) {
          if (error instanceof Error && error.message.includes('timed out')) {
            logger.warn('Session fetch timed out, clearing local session to recover');
            setUser(null);
            // Attempt to clean up potential stale state
