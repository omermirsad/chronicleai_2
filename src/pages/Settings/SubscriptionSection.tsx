...
const WaitlistFeature: React.FC<{
  ...
  React.useEffect(() => {
    const checkStatus = async () => {
      try {
        const { data, error } = await (supabase as any).rpc('check_waitlist_status', {
  ...
  return (
    <div className="flex items-start justify-between bg-white rounded-lg p-3 border border-blue-100">
      ...
      <button
        ...
        className={`ml-3 px-3 py-1.5 text-xs font-medium rounded-md transition flex-shrink-0 ${onWaitlist
